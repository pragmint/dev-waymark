...
export async function entitiesHandler(c: Context) {
  ...
  if (selectedEntityType) {
    ...
    // allIds is only materialised for regex trees — reuse it there; every
    // other tree aggregates in SQL without ids leaving the database.
    availableFilters = pagedResult.allIds
      ? await repo.getAvailableFilters(pagedResult.allIds, { allDistinctValues })
      : await repo.getAvailableFiltersForTree(activeTree, { allDistinctValues });
  } else {
    ...
    availableFilters = await repo.getAvailableFiltersForTree(emptyTree());
  }
