...
describe('entityRepository', () => {
  ...
  it('retrieves an entity with metadata', () => {
    repo.upsert(makeEntity(), [makeMetadata(1)]);
    const result = repo.get(1);
  ...
  it('lists all entities', () => {
    repo.upsert(makeEntity({ id: 1, name: 'A-1' }), []);
    repo.upsert(makeEntity({ id: 2, name: 'B-1' }), []);
  ...
  it('filters entities by metadata eq', () => {
    repo.upsert(makeEntity({ id: 1, name: 'J-1' }), [
      makeMetadata(1, { key: 'source', value: 'jira' }),
    ]);
    repo.upsert(makeEntity({ id: 2, name: 'L-1' }), [
  ...
  it('filters entities by metadata contains', () => {
    ...
    repo.upsert(makeEntity({ id: 2, name: 'J-2' }), [
  ...
  it('filters entities by numeric metadata gte', () => {
    repo.upsert(makeEntity({ id: 1, name: 'A' }), [
      makeMetadata(1, { key: 'total-wip', value: '40', value_type: 'number' }),
    ]);
    repo.upsert(makeEntity({ id: 2, name: 'B' }), [
  ...
  it('filters entities by date metadata range', () => {
    ...
    repo.upsert(makeEntity({ id: 2, name: 'C' }), [
      makeMetadata(2, {
        key: 'started-date',
  ...
  it('upserts without duplicating on re-insert', () => {
    repo.upsert(makeEntity(), [makeMetadata(1, { key: 'type', value: 'ticket' })]);
    repo.upsert(makeEntity({ name: 'ENG-1-updated' }), [
  ...
  it('filters entities by multiple eq values on same key (OR / IN)', () => {
    ...
    repo.upsert(makeEntity({ id: 3, name: 'G-1' }), [
  ...
  it('filters entities by regex', () => {
    ...
    repo.upsert(makeEntity({ id: 3, name: 'C' }), [
  ...
  describe('getAvailableFilters', () => {
    ...
    it('sets correct value_type for each key', () => {
      repo.upsert(makeEntity({ id: 1, name: 'X' }), [
